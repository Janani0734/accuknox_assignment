...
accuknox_assignment/
├── manage.py
├── accuknox_assignment/        # Project settings & URL conf
...
def q1_sync_proof_handler(sender, instance, **kwargs):
    time.sleep(2)   # deliberately blocks the caller

# tests.py
start   = time.perf_counter()
MyModel.objects.create(name="q1_sync_test")
elapsed = time.perf_counter() - start
assert elapsed >= 2.0   # passes → synchronous
```
...
The default behaviour (firing before commit) creates a race condition in real applications: if the signal enqueues a Celery task or calls an external API, the worker may try to read a row that has not been committed yet.
...
yield inside __iter__ is the cleanest approach — Python turns the method into a generator function which returns a fresh iterator on every call. The rectangle.py file also includes an explicit iterator version (Option B) as a reference to show the trade-off between the two approaches.
